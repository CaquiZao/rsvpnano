...
class DrivePoller:
    ...
    def poll_once(self) -> int:
        ...
        # Primeiro o sidecar órfão -- um .json que nenhum .wav da pasta vai
        # casar. É metadado, não gravação, então apagá-lo não destrói nada, e
        # vem antes das notas prontas para que um download que falhe lá embaixo
        # e aborte o poll não custe a limpeza deste ciclo.
        #
        # É a única remoção que não vem de uma nota entregue ou recusada. Os
        # arquivos de um note_id já processado ficam na pasta, intocados: ver
        # drive_inbox.InboxPlan -- apagá-los destruía gravação numa colisão de
        # stem boot-, e a paginação de list_inbox é o que impede a pasta cheia
        # de esconder uma gravação nova.
